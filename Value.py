...
class ValueFunc(object):
    ...
    def V_net(self, inputs, n, act_fns, name=''):
        
        with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
            for i in range(len(n)):
                if i == 0:
                    with tf.variable_scope(name+'w_1', reuse=tf.AUTO_REUSE):
                        for j, x in enumerate(inputs):
                            w = tf.get_variable(name='input_'+str(j), shape=(x.shape[-1 ], self.layer_sizes[i]),
                                                initializer=tf.contrib.layers.xavier_initializer())
                            out = tf.matmul(x, w)
                else:
                    w = tf.get_variable(name='w_'+str(i), shape=(self.layer_sizes[i-1], self.layer_sizes[i]),
                                        initializer=tf.contrib.layers.xavier_initializer()) # tf.random_normal_initializer() tf.contrib.layers.xavier_initializer()
                    out = tf.matmul(out, w)
                    
                b = tf.get_variable(name='b_'+str(i), shape=(self.layer_sizes[i]),
